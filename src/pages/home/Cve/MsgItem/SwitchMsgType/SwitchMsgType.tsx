...
const SwitchMsgType: FC<SwitchMsgTypeProps> = ({ msg, audio, curCve, selfID, imgClick }) => {
  ...
  const parsePictureUrl = (url: string | undefined) => {
    if (url) {
      return url.replace("127.0.0.1", "114.116.94.189");
    }
    return url;
  };
  useEffect(() => {
  ...
  const msgType = () => {
    switch (msg.contentType) {
      ...
      case messageTypes.PICTUREMESSAGE:
        const parsedPictureElem = {
          ...msg.pictureElem,
          snapshotPicture: {
            ...msg.pictureElem.snapshotPicture,
            url: parsePictureUrl(msg.pictureElem.snapshotPicture.url) ?? "",
          },
          sourcePicture: {
            ...msg.pictureElem.sourcePicture,
            url: parsePictureUrl(msg.pictureElem.sourcePicture.url) ?? "",
          },
          bigPicture: {
            ...msg.pictureElem.bigPicture,
            url: parsePictureUrl(msg.pictureElem.bigPicture.url) ?? "",
          },
        };
        return (
          <div className={`chat_bg_msg_content_pic ${!isSingle ? "nick_magin" : ""}`}>
            <Image
              placeholder={true}
              height={200}
              src={parsedPictureElem.snapshotPicture.url ?? parsedPictureElem.sourcePicture.url}
              preview={{ visible: false }}
              onClick={() => imgClick(parsedPictureElem)}
